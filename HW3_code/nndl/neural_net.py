...
def softmax(y):
  return np.exp(y)/np.reshape(np.sum(np.exp(y), axis=1), (len(y), 1))
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss += np.sum(np.log(np.sum(np.exp(scores), axis = 1)) - scores[np.arange(N), y])/N
    ...
    dLds /= N
    grads['W2'] = dLds.T@h1 + reg*W2
    grads['b2'] = dLds.T@np.ones(len(h1))
    dhdv = np.where(X@W1.T > 0, 1, 0)
    dd = np.multiply(dhdv, dLds@W2)
    grads['W1'] = dd.T @ X + reg*W1
    grads['b1'] = dd.T @ np.ones(len(X))
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in np.arange(num_iters):
      ...
      # ================================================================ #
      samps = np.random.choice(np.arange(len(X)), batch_size)
      X_batch = X[samps]
      y_batch = y[samps]
      # ================================================================ #
      # END YOUR CODE HERE
