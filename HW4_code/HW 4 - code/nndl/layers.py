...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        zero_x = x-sample_mean
        std_inv = 1/np.sqrt(sample_var + eps)
        x_hat = (zero_x)* std_inv
        
        out = gamma * x_hat + beta
        
        cache = {'x_hat': x_hat, 'gamma': gamma, 'std_inv': std_inv, 'zx': zero_x}
        # ================================================================ #
        # END YOUR CODE HERE
        # ================================================================ #
    elif mode == 'test':
        ...
        x_hat = (zero_x)/np.sqrt(running_var + eps)
...
def batchnorm_backward(dout, cache):
    ...
    de = -0.5 *cache['std_inv']**3 * cache['zx'] * dx_hat
    ds = np.sum(de, axis = 0)

    da = cache['std_inv']*dx_hat
    db = 2/N * cache['zx']*ds
    du = -np.sum(da, axis = 0) - np.sum(db, axis = 0)
